...
vec2 equirect_proj(float u, float v,float W,float H,float fov){
	float theta_alt = 2.0 * M_PI * (u - 0.5); //-90 degree ~ +90
	float phi_alt =  M_PI * (v - 0.5); //-90 degree ~ +90
...
void main() {
	...
	if(isPerspective==1){
		...
		float nx = texCoord.x-0.5;
		float ny = texCoord.y-0.5;
		nx *= 2;
		ny *= 2;
		...
		outColor = texture(tex0,vec2(u,v));
	}
	else{
		u = texCoord.x*width + offset;
		v = texCoord.y;
